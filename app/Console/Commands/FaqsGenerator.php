...
class FaqsGenerator extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'faqs:generator 
     ...
     * @var string
     */
    protected $description = 'Create FAQS operations';
    ...
    {
        return file_get_contents(resource_path("stubs2/$type.stub"));
    }
    ...
    protected function model($name, $table)
    {
        ...
        if (!file_exists($path = app_path("/Models/Faq/{$name}"))) {
            mkdir($path, 0777, true);
        }

        file_put_contents(app_path("/Models/Faq/{$name}/{$name}.php"), $modelTemplate);
    }
    ...
    protected function repository($name, $table)
    {
        ...
        file_put_contents(app_path("/Models/Faq/{$name}/{$name}Repository.php"), $modelTemplate);
    }
    ...
    protected function getBasePaths($table, $kebabLowerCase)
    {
        ...
        foreach ($columns as $key => $column) {
            if ($this->isFile($column)) {
                ...
                $template1 .= "protected \$basePath".$no." = '/files/faqs/".$kebabLowerCase."/".$newColumn."';\n";
    ...
    protected function transformer($name, $table)
    {
        ...
        file_put_contents(app_path("/Models/Faq/{$name}/{$name}Transformer.php"), $modelTemplate);
    }
    ...
    protected function controller($name, $table)
    {
        ...
        file_put_contents(app_path("/Http/Controllers/Faq/{$name}Controller.php"), $controllerTemplate);
    }
