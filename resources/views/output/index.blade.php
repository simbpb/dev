   ...
   $('#datatable').DataTable({
      ...
      "columns": [
         ...
         { "title" : "", "orderable": false, "width": "170px", "className": "text-center", render: function (data, type, row, meta) {
            ...
            var dele = '';
            @can('output_view')
               view = '<a href="{{ Navigation::adminUrl('/output') }}/'+row.id+'/view" class="btn btn-info btn-xs" data-toggle="tooltip" data-placement="top" title="Lihat"><i class="icon-eye"></i></a>';
            @endcan
            @can('output_edit')
               edit = '<a href="{{ Navigation::adminUrl('/output') }}/'+row.id+'/edit" class="btn btn-primary btn-xs" data-toggle="tooltip" data-placement="top" title="Ubah"><i class="icon-pencil"></i></a>';
            @endcan
            @can('output_delete')
               dele = '<button class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="top" title="Hapus" onclick="deleteRow(\'{{ Navigation::adminUrl('/output') }}/'+row.id+'/delete\',\'{{ Navigation::adminUrl('/output') }}\');"><i class="icon-eraser3"></i></button>';
            @endcan
      ...
      <div class="content-wrapper">
         <div class="panel panel-flat">
            ...
            <div class="panel-body">
               ...
               @can('output_create')
                  <a href="{{ Navigation::adminUrl('/output') }}/create" class="btn btn-primary btn-xs">
