   ...
   $('#datatable').DataTable({
      ...
      "columns": [
         ...
         { "title" : "", "orderable": false, "width": "170px", "className": "text-center", render: function (data, type, row, meta) {
            ...
            var dele = '';
            @can('volume_view')
               view = '<a href="{{ Navigation::adminUrl('/volume') }}/'+row.id+'/view" class="btn btn-info btn-xs" data-toggle="tooltip" data-placement="top" title="Lihat"><i class="icon-eye"></i></a>';
            @endcan
            @can('volume_edit')
               edit = '<a href="{{ Navigation::adminUrl('/volume') }}/'+row.id+'/edit" class="btn btn-primary btn-xs" data-toggle="tooltip" data-placement="top" title="Ubah"><i class="icon-pencil"></i></a>';
            @endcan
            @can('volume_delete')
               dele = '<button class="btn btn-danger btn-xs" data-toggle="tooltip" data-placement="top" title="Hapus" onclick="deleteRow(\'{{ Navigation::adminUrl('/volume') }}/'+row.id+'/delete\',\'{{ Navigation::adminUrl('/volume') }}\');"><i class="icon-eraser3"></i></button>';
            @endcan
      ...
      <div class="content-wrapper">
         <div class="panel panel-flat">
            ...
            <div class="panel-body">
               ...
               @can('volume_create')
                  <a href="{{ Navigation::adminUrl('/volume') }}/create" class="btn btn-primary btn-xs">
